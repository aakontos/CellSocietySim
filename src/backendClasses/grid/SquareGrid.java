...
/**
 * @author shichengrao
 * This class models a square tiling
...
public class SquareGrid implements Grid {
	...
	private static final List<int[]> xneighborhoods = new ArrayList<int[]>() {{
		add(new int[] {0,0,-1,1});
		add(new int[] {-1,-1,1,1,0,0,-1,1});
		add(new int[] {-1,-1,1,1});
		add(new int[] {0});
	}};
	private static final List<int[]> yneighborhoods = new ArrayList<int[]>() {{
		add(new int[] {-1,1,0,0});
		add(new int[] {-1,1,-1,1,-1,1,0,0});
		add(new int[] {-1,1,-1,1});
		add(new int[] {1});
	}};
	private boolean toroidal;
	/**
	 * Constructor for SquareGrid class, written to specify
	 * parameters
	 * @param width
	 * @param height
	 * @param g
	 * @param toroidal
	 */
	public SquareGrid(int width, int height, GameType g) {
	...
	public void updateGrid() {
		...
		for(int i = 0; i < grid.length; i++) {
			for(int j = 0; j < grid[0].length;j++) {
				if(validTile(i,j)) {
					List<Cell> pointNeighbors = getCells(xneighborhoods.get(neighborhoodType),yneighborhoods.get(neighborhoodType),i,j);
					Point2D point = game.determinePosition(grid[i][j], pointNeighbors, i, j, grid,xneighborhoods.get(neighborhoodType),yneighborhoods.get(neighborhoodType),toroidal);
					if(point == null || !validTile((int)point.getX(), (int)point.getY())) {
						continue;
					}
					grid[(int)point.getX()][(int)point.getY()].setNextState(game.determineState(grid[i][j],pointNeighbors));
